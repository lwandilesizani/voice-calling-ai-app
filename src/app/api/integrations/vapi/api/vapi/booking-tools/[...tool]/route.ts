...
export async function POST(
  ...
  try {
    // Make sure to await params.tool to avoid the sync access error
    const toolPath = Array.isArray(params.tool) ? params.tool.join('/') : params.tool;
    console.log(`Proxying request from incorrect path to correct path for ${toolPath}`);
    ...
    const response = await fetch(`${baseUrl}/api/vapi/booking-tools/${toolPath}`, {
      method: 'POST',
      headers,
      body: request.body,
      duplex: 'half'
